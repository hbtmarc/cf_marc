...
function cardPanelThemeKey(card: Card): string {
  const name = card.name.toLowerCase();
  if (name.includes("nubank")) {
    return "nubank";
  }
  if (name.includes("ourocard") || name.includes("bb ") || name.startsWith("bb")) {
    return "ourocard";
  }
...
function cardPanelNumberLabel(card: Card): string {
  if (card.last4?.trim()) {
    return `•••• •••• •••• ${escapeHtml(card.last4.trim())}`;
  }
  return "•••• •••• •••• ••••";
}

interface CardPanelDisplayState {
  label: string;
  chipModifier: "paid" | "open" | "overdue" | "credit" | "partial";
  surfaceTint: "paid" | "open" | "overdue" | "credit" | "partial";
  heroTone: BalanceTone;
}

function cardPanelDisplayState(
  invoice: Invoice,
  today = new Date().toISOString().slice(0, 10),
): CardPanelDisplayState {
...
function renderCardPanelStatus(invoice: Invoice): string {
  const state = cardPanelDisplayState(invoice);
  return `<span class="card-panel__status-chip card-panel__status-chip--${state.chipModifier}">${escapeHtml(state.label.toUpperCase())}</span>`;
}

function cardPanelHeroMoneyTone(state: CardPanelDisplayState): BalanceTone {
  if (state.surfaceTint === "open" || state.surfaceTint === "partial") {
    return "neutral";
  }
  return state.heroTone;
}

function renderCardPanelMetrics(invoice: Invoice): string {
  const state = cardPanelDisplayState(invoice);
  const total = invoiceTotalCentsValue(invoice);
  const moneyTone = cardPanelHeroMoneyTone(state);
  const credit = invoice.creditBalanceCents ?? 0;
  const creditInline =
    invoiceHasCredit(invoice) && credit > 0
      ? `
      <span class="card-panel__hero-credit">
        <span class="card-panel__hero-credit-label">Saldo positivo</span>
        <span class="card-panel__hero-credit-value money money--positive">${renderNominalMoney(credit, "positive")}</span>
      </span>`
      : "";

  return `
    <div class="card-panel__hero card-panel__hero--${state.surfaceTint}">
      <span class="card-panel__hero-label">${escapeHtml(state.label)}</span>
      <div class="card-panel__hero-amounts">
        <span class="card-panel__hero-value money money--${moneyTone}">${renderNominalMoney(total, moneyTone)}</span>${creditInline}
      </div>
    </div>
  `;
}
...
function renderProjectedStatusChip(): string {
  return `<span class="status-chip status-chip--projected">${escapeHtml(PROJECTED_STATUS_LABEL.toUpperCase())}</span>`;
}
...
export function renderInvoiceTableRow(input: {
  ...
  const projected = isProjectedInvoice(invoice);
  const statusLabel = invoiceStatusLabel(invoice);
  ...
  const h = (columnId: string): string => tableCellHeaders(tableId, columnId);
  const faturaLabel = projected
    ? "Fatura projetada"
    : `Fatura ${formatCompetenceLabel(invoice.competenceMonth)}`;

  return `
    <tr class="cfm-table__row--invoice${projected ? " cfm-table__row--projected" : ""}" data-invoice-row="${escapeHtml(invoice.id)}">
      ...
      <td class="cfm-table__cell--fatura" ${h("fatura")} data-label="Fatura">
        <span class="data-table__primary">${escapeHtml(faturaLabel)}</span>
        ${projected ? `<span class="data-table__secondary">${escapeHtml(formatCompetenceLabel(invoice.competenceMonth))}</span>` : ""}
      </td>
      ...
      <td class="cfm-table__cell--status" ${h("status")} data-label="Status">${projected ? renderProjectedStatusChip() : renderStatusChip(statusLabel, statusVariant)}</td>
...
export function renderInvoiceProjectedLineRow(
  data: AppData,
  item: ProjectedInstallment,
  tableId: string = TABLE_IDS.invoiceDetail,
): string {
  ...
  return `
    <tr class="cfm-table__row--projected" data-projected-id="${escapeHtml(item.id)}">
      ...
      <td class="cfm-table__cell--amount" ${h("amount")} data-label="Valor">${renderNominalMoney(item.amountCents)}</td>
    </tr>
  `;
}
...
export function renderInvoiceDetailPanel(input: {
  ...
  const closingLabel = invoice.closingDate
    ...
    : "—";
  const lineCount = transactions.length + projections.length;
      ...
      : `
        ...
        <table class="cfm-table cfm-table--invoice-lines" aria-label="Lançamentos da fatura" data-sort-table="${escapeHtml(mobileSortControlId)}">
          ...
          <tbody>
            ...
            ${projections.map((item) => renderInvoiceProjectedLineRow(data, item)).join("")}
          </tbody>
        </table>`;

  const meta = projected
    ? `${escapeHtml(formatCompetenceLabel(invoice.competenceMonth))} · Vencimento ${escapeHtml(formatDateLabel(invoice.dueDate))}`
    : `${escapeHtml(formatCompetenceLabel(invoice.competenceMonth))} · Fechamento ${escapeHtml(closingLabel)} · Vencimento ${escapeHtml(formatDateLabel(invoice.dueDate))}`;
    ...
    <section class="invoice-detail" id="${escapeHtml(panelId)}" aria-labelledby="${escapeHtml(panelId)}-title">
      <header class="invoice-detail__header">
        <div class="invoice-detail__heading">
          <h3 class="invoice-detail__title" id="${escapeHtml(panelId)}-title">${escapeHtml(title)} · ${escapeHtml(cardName)}</h3>
          <p class="invoice-detail__meta">${meta}</p>
        </div>
        <div class="invoice-detail__status">${projected ? renderProjectedStatusChip() : renderStatusChip(statusLabel, statusVariant)}</div>
      </header>
      <dl class="invoice-detail__summary">${summaryItems}</dl>
      <p class="invoice-detail__count">${escapeHtml(formatTransactionCount(lineCount))} observado${lineCount === 1 ? "" : "s"}${projected ? " · compromissos projetados a partir das parcelas em aberto" : ""}</p>
      ${linesBody}
    </section>
  `;
}
...
export function renderCardPanel(input: {
  ...
  const { card, invoice, single = false, competenceMonth } = input;
  ...
  const theme = cardPanelThemeKey(card);
  const displayState = invoice ? cardPanelDisplayState(invoice) : null;

  const stateClass = invoice ? "card-panel--has-invoice" : "card-panel--empty";
  const tintIntensityClass =
    competenceMonth !== undefined &&
    competenceMonth === currentCompetenceMonth()
      ? "card-panel--tint-current"
      : "card-panel--tint-emphasis";

  const body = invoice
    ? renderCardPanelMetrics(invoice)
    : `<p class="card-panel__empty">Nenhuma fatura registrada para este cartão na competência.</p>`;

  return `
    <article class="card-panel card-panel--theme-${theme}${single ? " card-panel--single" : ""} ${tintIntensityClass} ${stateClass}">
      ${invoice ? `<div class="card-panel__status">${renderCardPanelStatus(invoice)}</div>` : ""}
      <div class="card-panel__surface">
        <div class="card-panel__decor" aria-hidden="true">
          <span class="card-panel__decor-ring card-panel__decor-ring--one"></span>
          <span class="card-panel__decor-ring card-panel__decor-ring--two"></span>
        </div>
        ${displayState ? `<div class="card-panel__state-tint card-panel__state-tint--${displayState.surfaceTint}" aria-hidden="true"></div>` : ""}
        <div class="card-panel__top">
          <div class="card-panel__chip" aria-hidden="true"></div>
        </div>
        <div class="card-panel__identity">
          <div class="card-panel__identity-text">
            <h3 class="card-panel__name"${nameAttr}>${escapeHtml(card.name)}</h3>
          </div>
          <p class="card-panel__pan">${cardPanelNumberLabel(card)}</p>
        </div>
        ${body}
        <footer class="card-panel__footer">
          <span class="card-panel__cycle">${escapeHtml(cycle)}</span>
        </footer>
      </div>
    </article>
  `;
